...
import { User, Recipe, Meal, Course, Preparation, Classification, Source, RecipeLike } from "@/db/models"
...
async function seedUser() {
  const id = uid()
  return new User({ email: `user-${id}@recipe.test`, username: `rcp-${id}`, displayUsername: `User ${id}` }).save()
}
  ...
  describe("visibility filtering", () => {
    it("anon user sees a public recipe but not a private recipe from the same owner", async () => {
      await withCleanDb(async () => {
        ...
        await new Recipe({ name: "Public One", userId: owner.id, isPublic: true }).save()
        await new Recipe({ name: "Private One", userId: owner.id, isPublic: false }).save()
    ...
    it("authenticated owner sees their own private recipe", async () => {
      await withCleanDb(async () => {
        const owner = await seedUser()
        await new Recipe({ name: "My Private", userId: owner.id, isPublic: false }).save()
    ...
    it("anon user sees nothing when scoped to an owner who has only private recipes", async () => {
      await withCleanDb(async () => {
        const owner = await seedUser()
        await new Recipe({ name: "Hidden", userId: owner.id, isPublic: false }).save()
    ...
    it("another authenticated user cannot see a private recipe through userId scoping", async () => {
      await withCleanDb(async () => {
        ...
        await new Recipe({ name: "Top Secret", userId: secretOwner.id, isPublic: false }).save()
  ...
  describe("search", () => {
    it("filters by name using case-insensitive partial match within a user's recipes", async () => {
      await withCleanDb(async () => {
        ...
        await new Recipe({ name: "Pasta Carbonara", userId: user.id, isPublic: true }).save()
        await new Recipe({ name: "Beef Stew", userId: user.id, isPublic: true }).save()
    ...
    it("filters by ingredients text (>= 2 chars) within a user's recipes", async () => {
      await withCleanDb(async () => {
        const user = await seedUser()
        await new Recipe({ name: "Soup", userId: user.id, isPublic: true, ingredients: "chicken broth\nnoodles" }).save()
        await new Recipe({ name: "Salad", userId: user.id, isPublic: true, ingredients: "lettuce\ntomato" }).save()
    ...
    it("whitespace-only search returns all visible recipes without applying any filter", async () => {
      await withCleanDb(async () => {
        const user = await seedUser()
        await new Recipe({ name: "Recipe A", userId: user.id, isPublic: true }).save()
        await new Recipe({ name: "Recipe B", userId: user.id, isPublic: true }).save()
  ...
  describe("filter parameters", () => {
    it("filters by mealId — returns only recipes linked to that meal", async () => {
      await withCleanDb(async () => {
        const user = await seedUser()
        const id = uid()
        const meal = await new Meal({ name: "Dinner", slug: `dinner-${id}` }).save()
        const recipeWithMeal = await new Recipe({ name: "Steak", userId: user.id, isPublic: true, mealIds: [meal.id] }).save()
        await new Recipe({ name: "Salad", userId: user.id, isPublic: true }).save()
...
describe("recipes.list — classificationName", () => {
  it("includes classificationName when recipe has a linked classification", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const id = uid()
      const cls = await new Classification({ name: "Italian", slug: `italian-${id}` }).save()
      await new Recipe({ name: "Pasta", userId: user.id, isPublic: true, classificationId: cls.id }).save()
  ...
  it("returns classificationName as null when recipe has no classification", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      await new Recipe({ name: "Plain Recipe", userId: user.id, isPublic: true }).save()
...
describe("recipes.byId", () => {
  ...
  it("returns a public recipe when found", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const recipe = await new Recipe({ name: "Found Recipe", userId: user.id, isPublic: true }).save()
  ...
  it.each([
    {
      ...
      link: async (recipeId: string) => {
        const id = uid()
        const m = await new Meal({ name: "Breakfast", slug: `bfast-${id}` }).save()
        await Recipe.findByIdAndUpdate(recipeId, { $addToSet: { mealIds: m.id } })
        return m
      },
    ...
    },
    {
      ...
      link: async (recipeId: string) => {
        const id = uid()
        const c = await new Course({ name: "Entree", slug: `entree-${id}` }).save()
        await Recipe.findByIdAndUpdate(recipeId, { $addToSet: { courseIds: c.id } })
        return c
      },
    ...
    },
    {
      ...
      link: async (recipeId: string) => {
        const id = uid()
        const p = await new Preparation({ name: "Bake", slug: `bake-${id}` }).save()
        await Recipe.findByIdAndUpdate(recipeId, { $addToSet: { preparationIds: p.id } })
        return p
      },
      resultKey: "preparations" as const,
    },
  ])("returns $label names when recipe has linked $label", async ({ link, resultKey }) => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const recipe = await new Recipe({ name: "Dish", userId: user.id, isPublic: true }).save()
      const entity = await link(recipe.id)
      const result = await (await makeAnonCaller()).recipes.byId({ id: recipe.id })
      expect(result?.[resultKey]).toEqual(
        expect.arrayContaining([expect.objectContaining({ id: entity.id, name: entity.name })]),
      )
    })
  })
  ...
  it.each([
    {
      ...
      seed: async () => {
        const id = uid()
        const cls = await new Classification({ name: "Italian", slug: `italian-${id}` }).save()
        return { recipeExtra: { classificationId: cls.id }, expected: { classificationName: "Italian" } }
      },
    },
    {
      ...
      seed: async () => {
        ...
        return { recipeExtra: { sourceId: src.id }, expected: { sourceName: "Bon Appétit", sourceUrl: "https://bonappetit.com" } }
      },
    },
    ...
    await withCleanDb(async () => {
      ...
      const recipe = await new Recipe({ name: "Dish", userId: user.id, isPublic: true, ...recipeExtra }).save()
      expect(await (await makeAnonCaller()).recipes.byId({ id: recipe.id })).toMatchObject(expected)
    })
  })
  ...
  it.each([
    ["classificationName"],
    ["sourceName"],
  ])("returns %s as null when recipe has no linked entity", async (field) => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const recipe = await new Recipe({ name: "Plain", userId: user.id, isPublic: true }).save()
      expect(await (await makeAnonCaller()).recipes.byId({ id: recipe.id })).toMatchObject({ [field]: null })
    })
  })
})
...
describe("recipes.update", () => {
  it("rejects unauthenticated requests", async () => {
    await withCleanDb(async () => {
      const caller = await makeAnonCaller()
      await expect(caller.recipes.update({ id: VALID_OBJECT_ID, name: "Updated" })).rejects.toThrow("UNAUTHORIZED")
    })
  })
  ...
  it("updates the recipe name", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const recipe = await new Recipe({ name: "Old Name", userId: user.id }).save()
  ...
  it("returns current record unchanged when only taxonomy IDs are updated", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const id = uid()
      const recipe = await new Recipe({ name: "Existing", userId: user.id }).save()
      const meal = await new Meal({ name: "Breakfast", slug: `bfast-${id}` }).save()
...
describe("recipes.delete", () => {
  it("rejects unauthenticated requests", async () => {
    await withCleanDb(async () => {
      const caller = await makeAnonCaller()
      await expect(caller.recipes.delete({ id: VALID_OBJECT_ID })).rejects.toThrow("UNAUTHORIZED")
    })
  })
  ...
  it("deletes the recipe and makes it no longer retrievable", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const recipe = await new Recipe({ name: "Delete Me", userId: user.id, isPublic: true }).save()
...
describe("recipes.list — hasImage filter", () => {
  it("returns only recipes with an imageUrl when hasImage is true", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      await new Recipe({ name: "With Image", userId: user.id, isPublic: true, imageUrl: "https://example.com/img.jpg" }).save()
      await new Recipe({ name: "No Image", userId: user.id, isPublic: true }).save()
...
describe("recipes.list — servings range filter", () => {
  it("minServings returns only recipes with servings >= min", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      await new Recipe({ name: "Small Batch", userId: user.id, isPublic: true, servings: 2 }).save()
      await new Recipe({ name: "Large Batch", userId: user.id, isPublic: true, servings: 8 }).save()
  ...
  it("minServings + maxServings returns recipes within the inclusive range", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      await new Recipe({ name: "Tiny", userId: user.id, isPublic: true, servings: 1 }).save()
      await new Recipe({ name: "Medium", userId: user.id, isPublic: true, servings: 4 }).save()
      await new Recipe({ name: "Huge", userId: user.id, isPublic: true, servings: 20 }).save()
  ...
  it("excludes recipes with null servings from a minServings query", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      await new Recipe({ name: "Unknown Servings", userId: user.id, isPublic: true }).save()
      await new Recipe({ name: "Known Servings", userId: user.id, isPublic: true, servings: 4 }).save()
...
describe("recipes.list — new sort options", () => {
  ...
  it("servings_asc orders by servings ascending", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      await new Recipe({ name: "Eight Servings", userId: user.id, isPublic: true, servings: 8 }).save()
      await new Recipe({ name: "Two Servings", userId: user.id, isPublic: true, servings: 2 }).save()
  ...
  it("updated_desc orders by updatedAt descending", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const r1 = await new Recipe({ name: "Old Recipe", userId: user.id, isPublic: true, updatedAt: new Date("2020-01-01") }).save()
      await new Recipe({ name: "New Recipe", userId: user.id, isPublic: true }).save()
...
describe("recipes.list — markedByMe filter", () => {
  it("returns only the recipe the caller has favorited", async () => {
    await withCleanDb(async () => {
      const owner = await seedUser()
      const viewer = await seedUser()
      const liked = await new Recipe({ name: "Liked Recipe", userId: owner.id, isPublic: true }).save()
  ...
  it("treats markedByMe as a no-op for anonymous callers and returns visible recipes", async () => {
    await withCleanDb(async () => {
      ...
      const publicRecipe = await new Recipe({ name: "Public Recipe", userId: owner.id, isPublic: true }).save()
      await new RecipeLike({ userId: liker.id, recipeId: publicRecipe.id }).save()
  ...
  it("does not include recipes favorited by a different user", async () => {
    await withCleanDb(async () => {
      ...
      const viewer = await seedUser()
      const recipe = await new Recipe({ name: "Shared Recipe", userId: owner.id, isPublic: true }).save()
      await new RecipeLike({ userId: otherUser.id, recipeId: recipe.id }).save()
...
describe("recipes.isMarked", () => {
  it("returns false for anonymous users without querying DB", async () => {
    await withCleanDb(async () => {
      const caller = await makeAnonCaller()
      expect(await caller.recipes.isMarked({ id: VALID_OBJECT_ID })).toEqual({ marked: false })
    })
  })
})
...
describe("recipes.toggleMarked", () => {
  it("rejects unauthenticated requests", async () => {
    await withCleanDb(async () => {
      const caller = await makeAnonCaller()
      await expect(caller.recipes.toggleMarked({ id: VALID_OBJECT_ID })).rejects.toThrow("UNAUTHORIZED")
    })
  })
  ...
  it("cycles through the full isMarked=false → true → false state", async () => {
    await withCleanDb(async () => {
      const user = await seedUser()
      const recipe = await new Recipe({ name: "Toggle Me", userId: user.id }).save()
