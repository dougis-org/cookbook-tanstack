...
function useModalA11y(onDismiss?: () => void) {
...
export default function TierWall({ reason, display, onDismiss }: TierWallProps) {
  ...
  return (
    <div
      ...
      role="dialog" aria-modal="true" aria-labelledby="tier-wall-title" aria-describedby="tier-wall-desc"
    >
      <div ref={modalRef} tabIndex={-1} className="bg-[var(--theme-surface)] border border-[var(--theme-border)] rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl outline-none">
        <h2 id="tier-wall-title" className="text-lg font-bold text-[var(--theme-fg)] mb-2">{title}</h2>
        <p id="tier-wall-desc" className="text-[var(--theme-fg-muted)] mb-6">{body}</p>
