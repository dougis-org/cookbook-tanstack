...
test.describe("Registration Form UI", () => {
  ...
  test("should display form field placeholders", async ({ page }) => {
    ...
    await expect(
      page.getByPlaceholder("Your display name (optional)"),
    ).toBeVisible();
    await expect(
      page.getByPlaceholder("Choose a unique username"),
    ).toBeVisible();
  ...
  test("should clear form fields when new values are entered", async ({
    page,
  }) => {
    const timestamp = Date.now();
    const usernameInput = page.getByLabel(/^Username/);
