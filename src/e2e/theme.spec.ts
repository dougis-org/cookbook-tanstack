...
test.describe('Theme system', () => {
  ...
  test('theme persists across page reload', async ({ page }) => {
    ...
    const htmlClassAfterLoad = await page.evaluate(
      () => document.documentElement.className,
    )
  ...
  test('T4b: selecting an option previews the theme before OK', async ({
    page,
  }) => {
    await gotoAndWaitForHydration(page, '/')
  ...
  test('T4c: pressing OK commits the theme and closes sidebar', async ({
    page,
  }) => {
    await gotoAndWaitForHydration(page, '/')
    ...
    await page.getByLabel('Open menu').click()
    await expect(page.getByTestId('theme-dropdown-trigger')).toContainText(
      'Light (warm)',
    )
  })

  // T4d: select light-cool, press Cancel — assert html class reverts
  test('T4d: pressing Cancel reverts preview and closes sidebar', async ({
    page,
  }) => {
    await gotoAndWaitForHydration(page, '/')
  ...
  // T4e: Escape key — assert dropdown closes, class reverts, and sidebar closes
  test('T4e: pressing Escape while preview active closes dropdown, reverts class, and closes sidebar', async ({
    page,
  }) => {
    await gotoAndWaitForHydration(page, '/')
  ...
  test('T4f: clicking outside dropdown reverts preview and closes sidebar', async ({
    page,
  }) => {
    await gotoAndWaitForHydration(page, '/')
  ...
  test('light-warm: active filter chip text matches the theme accent', async ({
    page,
  }) => {
    await page.addInitScript(() => {
    ...
    const hasImageBtn = page.getByRole('button', {
      name: 'Has Image',
      exact: true,
    })
  ...
  test('no flash: html has light-warm class before hydration when light-warm stored', async ({
    page,
  }) => {
    await page.addInitScript(() => {
      localStorage.setItem('cookbook-theme', 'light-warm')
  ...
  test('light-cool: active filter chip text matches the theme accent', async ({
    page,
  }) => {
    await page.addInitScript(() => {
      localStorage.setItem('cookbook-theme', 'light-cool')
  ...
  test('print isolation: PrintLayout wrapper has white background regardless of theme', async ({
    page,
  }) => {
