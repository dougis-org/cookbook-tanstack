...
- `src/components/recipes/RecipeDetail.tsx`: ingredient `<li>` and
  ...
  instruction step's existing numbered-circle `<span>` stays `print:hidden`
  (unchanged); the new print-only delimiter is instead generated by the
  shared `.print-list-item::before` CSS rule, not by making any existing
  `<span>` visible.
