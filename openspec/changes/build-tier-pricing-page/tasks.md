...
- [ ] Verify `src/routes/pricing.tsx`, `src/routes/upgrade.tsx`, `src/server/trpc/routers/usage.ts`, and account page tier section appear on `main`
- [ ] Mark all remaining tasks as complete
- [ ] No documentation updates required beyond code changes
- [ ] Sync approved spec deltas: copy `openspec/changes/build-tier-pricing-page/specs/*.md` into `openspec/specs/` (create dir if needed)
- [ ] Archive the change: move `openspec/changes/build-tier-pricing-page/` to `openspec/changes/archive/YYYY-MM-DD-build-tier-pricing-page/` — stage both copy and deletion in **one commit**
- [ ] Confirm `openspec/changes/archive/YYYY-MM-DD-build-tier-pricing-page/` exists and `openspec/changes/build-tier-pricing-page/` is gone
- [ ] Commit and push archive to `main` in one commit
- [ ] Prune merged branch: `git fetch --prune` and `git branch -d feat/tier-pricing-page`
