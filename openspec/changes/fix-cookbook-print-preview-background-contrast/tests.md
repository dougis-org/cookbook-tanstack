...
- [ ] **Test case 1 (task: RED — write/update failing tests; scenario: "TOC/print page background is light in the dark theme")** — In `src/components/cookbooks/__tests__/CookbookStandaloneLayout.test.tsx`, render `CookbookStandalonePage` and assert the rendered wrapper's className includes `bg-[var(--theme-print-bg)]`. Confirm this assertion fails against current code (which contains `bg-[var(--theme-bg)]` instead).

- [ ] **Test case 2 (task: RED — write/update failing tests; scenario: "TOC/print page background is light in the dark theme")** — In the same test file, assert the rendered wrapper's className does **not** include `bg-[var(--theme-bg)]`. Confirm this assertion fails against current code.

- [ ] **Test case 3 (task: GREEN — implement the fix; scenario: "TOC/print page background is light in the dark theme")** — After changing `pageBaseClass` to `'min-h-screen bg-[var(--theme-print-bg)]'`, re-run Test cases 1 and 2 and confirm both now pass.
