...
- Does `ask-sdk-express-adapter`'s signature-verification behavior work cleanly on top of Nitro/h3's request handling (via `fromNodeMiddleware` or equivalent), or does the skill route need a thinner, hand-adapted verification step against the same underlying `ask-sdk-core` primitives? To be confirmed during the discovery spike.
