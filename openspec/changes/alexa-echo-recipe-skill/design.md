...
- **[Risk] A self-hosted skill endpoint must correctly verify Alexa's request signature (`SignatureCertChainUrl` + timestamp) on every request; getting this wrong could let spoofed/replayed requests through.** → Mitigation: use `ask-sdk-express-adapter`, which implements this verification, rather than hand-rolling it; cover it with tests asserting unsigned/stale/mismatched requests are rejected. Signature verification alone only proves a request came from genuine Alexa infrastructure, not that it's addressed to *this* skill — anyone can point their own registered skill's endpoint at the same URL. Mitigation: additionally verify the request's `applicationId` against a configured `ALEXA_SKILL_ID`, failing closed if unconfigured (see `docs/alexa-skill-deployment.md`).
- **[Risk] `@better-auth/oauth-provider` 1.4.8–1.7.0-beta.3 (the installed 1.6.x line) has an unpatched moderate advisory, GHSA-p2fr-6hmx-4528: a caller holding a valid registered client's credentials can request an access token for an unintended audience via unbound resource indicators.** A fix exists only in the `1.7.0-beta.4+` pre-release line and requires a breaking `npx auth migrate` schema migration, not taken in this change. → Mitigation: `validAudiences` is pinned to this app's single `BETTER_AUTH_URL` in `src/lib/auth.ts` (there is no second audience for a token to be misdirected to), dynamic client registration is not enabled (only the manually-registered Alexa client can present valid credentials at all), and `src/server/alexa/token-validation.ts` independently re-verifies the `audience` claim on every request as defense-in-depth. Revisit once a non-beta patched release exists.
