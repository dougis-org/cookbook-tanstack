...
Issue #603 asks for bringing My CookBooks to Amazon smart-display devices (Echo Show, Fire TV-adjacent Echo Hub) so users can ask Alexa to find and display recipes hands-free while cooking. This is a discovery surface with real value: kitchen use is exactly the moment a phone/laptop is inconvenient, and voice + visual (APL) recipe cards are a natural fit for the existing recipe/cookbook data model. This change covers both the design and the implementation: account linking, the read-only adapter, the self-hosted skill route, and the APL documents all ship as part of this change. Amazon Developer Console registration and skill certification remain manual/external steps performed by a human, not code delivered here.
