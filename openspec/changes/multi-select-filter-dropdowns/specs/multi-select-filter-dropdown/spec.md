...
- **THEN** the button displays `"{n} {label}s"` (e.g., "2 categories")

### Requirement: Open a checkbox dropdown panel on button click
The system SHALL toggle a dropdown panel of checkbox options when the summary button is clicked, allowing users to select or deselect individual options.
