...
- [ ] 1.1 Search codebase for legacy collection references (`users`, `sessions`, `accounts`)
  - Run: `grep -r "users\|sessions\|accounts" src/ --include="*.ts" --include="*.tsx" --exclude-dir=node_modules`
  - Document findings in audit log
  
- [ ] 1.2 Verify no Mongoose models for User, Session, Account in `src/db/models/`
...
- [ ] 1.3 Audit `src/db/seeds/` for legacy collection initialization
...
- [ ] 1.4 Check tRPC routers and middleware for direct auth collection queries
...
- [ ] 1.5 Review environment variables and configuration files
...
- [ ] 2.1 Delete model definition files (if present)
...
- [ ] 2.2 Update barrel export in `src/db/models/index.ts`
...
- [ ] 3.1 Remove/update seed scripts in `src/db/seeds/`
...
- [ ] 3.2 Clean up `src/db/index.ts` (MongoDB connection)
...
- [ ] 3.3 Update Docker and initialization scripts
...
- [ ] 4.2 Create migration/cleanup notes
