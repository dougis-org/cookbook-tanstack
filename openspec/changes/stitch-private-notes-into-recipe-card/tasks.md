...
- [ ] 1.1 Grep `PrivateRecipeNotes.test.tsx`, `RecipeDetail.test.tsx` /
...
- [ ] 2.1 In `PrivateRecipeNotes.test.tsx`, add or extend tests asserting
...
- [ ] 2.2 Add/extend tests asserting the loaded-content and skeleton
...
- [ ] 2.3 Run the new/updated tests and confirm they fail against the
      current implementation (red, before any component change).

## 3. Implement the wrapper restyle

- [ ] 3.1 In `src/components/recipes/PrivateRecipeNotes.tsx`, wrap the
...
- [ ] 3.2 Update the loading-skeleton wrapper: add `max-w-4xl mx-auto`
...
- [ ] 3.3 Update the loaded/editing content wrapper (the final `return`
...
- [ ] 3.4 Re-run the tests from step 2 and confirm they now pass (green).

## 4. Manual visual verification

- [ ] 4.1 Start the dev server, view a recipe as a Sous Chef+ user with an
...
- [ ] 4.2 Repeat the check for the loading skeleton, and for each
...
- [ ] 4.3 Confirm width alignment specifically on a viewport wider than
      56rem (4xl), where the pre-fix mismatch was most visible (e.g. resize
      browser to ~1440px or wider).
- [ ] 4.4 Use the browser print preview on a recipe with a saved private
...
- [ ] 5.1 Run `npm run test` and confirm the full unit/integration suite
      passes.
- [ ] 5.2 Run the existing Playwright coverage relevant to the recipe
      detail page / private notes (if any) and confirm no regressions.
- [ ] 5.3 Update `.wolf/anatomy.md` and append to `.wolf/memory.md` per
