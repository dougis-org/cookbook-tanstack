...
- The Recipe document no longer exists in the `recipes` collection
...
| # | Given | When | Then |
|---|-------|------|------|
| AC1 | A recipe exists in one or more cookbooks | Owner deletes the recipe | All cookbook entries for that recipe are removed |
| AC2 | A recipe has one or more likes | Owner deletes the recipe | All `RecipeLike` docs for that recipe are removed |
| AC3 | A recipe exists with no cookbook or like references | Owner deletes the recipe | Delete succeeds with `{ success: true }` |
