...
- [ ] **T3 — Implement `src/routes/account.settings.tsx`**: `createFileRoute("/account/settings")`, `beforeLoad: requireAuth()`, theme selection form following `design-system/CLAUDE.md` (theme tokens only, Title Case labels, Lucide icons, no emoji). Save handler calls `authClient.updateUser({ theme })` directly (Design Decision 1) — no new tRPC procedure. Verify tests from T2 pass.
