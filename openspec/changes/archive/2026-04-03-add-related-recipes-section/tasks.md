...
- [x] 5.1 Commit all changes to `feature/add-related-recipes-section` and push
- [x] 5.2 Open PR from `feature/add-related-recipes-section` → `main` with title "feat: add related recipes section to recipe detail page"; enable auto-merge
- [x] 5.3 Monitor CI — if checks fail: diagnose, fix, commit, push, repeat until green
- [x] 5.4 Monitor review comments — address each, commit fixes, push, repeat until no unresolved comments remain
- [x] 5.5 Confirm auto-merge completes (or merge manually once all checks pass and comments resolved)

## 6. Post-Merge

- [x] 6.1 Checkout `main` and pull (`git checkout main && git pull --ff-only`)
- [x] 6.2 Verify merged changes appear on `main`
- [x] 6.3 Sync approved spec to `openspec/specs/related-recipes-section/spec.md` (copy from change specs)
- [x] 6.4 Archive change directory as a single atomic commit: copy `openspec/changes/add-related-recipes-section/` to `openspec/archive/add-related-recipes-section/` and delete the original in the same commit
