...
- **Chosen:** Load Fraunces and Inter via Google Fonts `<link>` tags in `src/routes/__root.tsx`. Use separate typography utilities: `.font-display` for regular headings (WONK 0) and `.brand-wordmark` for the application identity (WONK 1). Set Inter as the primary `--font-sans`.
