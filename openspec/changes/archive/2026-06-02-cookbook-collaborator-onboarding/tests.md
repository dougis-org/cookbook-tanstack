...
- [x] **Test Case 1.1: Verify default `onboarded` value**
...
- [x] **Test Case 1.2: Verify saving explicit `onboarded` values**
...
- [x] **Test Case 2.1: Verify `cookbooks.byId` output exposes `onboarded` status**
...
- [x] **Test Case 2.2: Verify `cookbooks.onboardCollaborator` updates state**
...
- [x] **Test Case 2.3: Verify `cookbooks.onboardCollaborator` access controls**
  ...
  - **Then** the procedure fails, throwing a `FORBIDDEN` error.
  - *Location*: `src/server/trpc/routers/__tests__/cookbooks.test.ts`

- [x] **Test Case 2.4: Verify `cookbooks.onboardCollaborator` is idempotent**
  - **Given** an onboarded collaborator logged in
  - **When** they execute the `cookbooks.onboardCollaborator` mutation
  - **Then** the mutation returns `{ success: true }` and the collaborator's `onboarded` status remains `true` in the database.
  - *Location*: `src/server/trpc/routers/__tests__/cookbooks.test.ts`

### Task 3 & 4: Build and Integrate Onboarding Modal UI

- [x] **Test Case 3.1: Verify Onboarding Modal renders correct Editor copy**
...
- [x] **Test Case 3.2: Verify Onboarding Modal renders correct Viewer copy**
...
- [x] **Test Case 4.1: Verify onboarding modal auto-triggers and dismisses correctly**
...
- [x] **Test Case 4.2: Verify onboarding modal does not show if collaborator is already onboarded**
  - **Given** a logged-in collaborator who is onboarded landing on the cookbook detail page
  - **When** the page finishes loading
  - **Then** the onboarding modal is not displayed.
  - *Location*: `src/components/cookbooks/__tests__/CookbookDetail.test.tsx`
