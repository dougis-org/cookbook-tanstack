...
- [x] **Step 2 — Create and publish working branch:** `git checkout -b feat/add-personal-source-name` then immediately `git push -u origin feat/add-personal-source-name`

## Execution

- [x] Task 1: Update `src/db/models/recipe.ts`
  - Add `personalSourceName?: string` to `IRecipe` interface.
  - Add `personalSourceName: { type: String, maxlength: 80 }` to `recipeSchema`.
- [x] Task 2: Update `src/db/models/__tests__/recipe.test.ts`
...
- [x] Task 3: Update `src/types/recipe.ts`
  - Add `personalSourceName: string | null` to the `Recipe` interface.
- [x] Task 4: Update `src/server/trpc/routers/recipes.ts`
...
- [x] Task 5: Update `src/lib/validation.ts`
...
- [x] Open PR from working branch to `main`. **If this change is issue-driven, the PR body MUST explicitly state "Closes #503" for each issue.**
...
- [x] Archive the change: move `openspec/changes/add-personal-source-name/` to `openspec/changes/archive/YYYY-MM-DD-add-personal-source-name/` **and stage both the new location and the deletion of the old location in a single commit** — do not commit the copy and delete separately
- [x] Confirm `openspec/changes/archive/YYYY-MM-DD-add-personal-source-name/` exists and `openspec/changes/add-personal-source-name/` is gone
- [x] **Create a doc branch** for the archive and spec updates: `git checkout -b doc/archive-YYYY-MM-DD-add-personal-source-name` then `git push -u origin doc/archive-YYYY-MM-DD-add-personal-source-name`
- [x] Open a PR from `doc/archive-YYYY-MM-DD-add-personal-source-name` to `main` with title `docs: archive add-personal-source-name (YYYY-MM-DD)` — **do NOT push directly to `main`**
...
- [x] Prune merged local branches: `git fetch --prune` and `git branch -D feat/add-personal-source-name doc/archive-YYYY-MM-DD-add-personal-source-name`
