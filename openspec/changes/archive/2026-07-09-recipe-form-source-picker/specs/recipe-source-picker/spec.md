...
### Requirement: ADDED Sorted, paginated Source dropdown in the recipe edit form

The system SHALL render the recipe edit/create form's Source field as a click-to-open, alphabetically-sorted dropdown (`RecipeSourcePicker`), consistent with the Category field's interaction pattern, backed by server-side pagination rather than a fully client-loaded list.
...
### Requirement: ADDED Personal Source Name field in the recipe source picker
...
### Requirement: ADDED Standalone Add New Source action
...
### Requirement: MODIFIED sourcesRouter supports bounded, sorted pagination
...
- **Proposal element -> Requirement**:
  - Recipe edit form Source field uses the same sorted-dropdown pattern as Category -> ADDED Sorted, paginated Source dropdown in the recipe edit form
  - Keep filtering server-side; load first 100, paginate further via infinite scroll -> ADDED Sorted, paginated Source dropdown in the recipe edit form; MODIFIED sourcesRouter supports bounded, sorted pagination
  - Bump `sources.search`'s limit with explicit validation -> MODIFIED sourcesRouter supports bounded, sorted pagination
  - Preserve personal-source-name behavior -> ADDED Personal Source Name field in the recipe source picker
  - Move "create new source" to a standalone button + modal, source-only -> ADDED Standalone Add New Source action
  - Do not change `sources.list` or the recipes filter bar -> MODIFIED sourcesRouter supports bounded, sorted pagination (scenario: sources.list and the recipes filter bar are unaffected)
- **Design decision -> Requirement**:
  - Decision 1 (`sources.listPage`) -> MODIFIED sourcesRouter supports bounded, sorted pagination
  - Decision 2 (`sources.search` bounds) -> MODIFIED sourcesRouter supports bounded, sorted pagination
  - Decision 3 (`RecipeSourcePicker` / `PaginatedSingleSelectDropdown`) -> ADDED Sorted, paginated Source dropdown in the recipe edit form; ADDED Personal Source Name field in the recipe source picker
  - Decision 4 (`AddSourceModal`) -> ADDED Standalone Add New Source action
- **Requirement -> Task(s)**:
  - MODIFIED sourcesRouter supports bounded, sorted pagination -> Task: Add `sources.listPage` procedure and tests; Task: Bound `sources.search` inputs and tests
  - ADDED Sorted, paginated Source dropdown in the recipe edit form -> Task: Implement `PaginatedSingleSelectDropdown`; Task: Implement `RecipeSourcePicker`
  - ADDED Personal Source Name field in the recipe source picker -> Task: Port personal-name logic into `RecipeSourcePicker`
  - ADDED Standalone Add New Source action -> Task: Implement `AddSourceModal`; Task: Wire `RecipeForm.tsx` to `RecipeSourcePicker` and `AddSourceModal`
...
> Invalid pagination/search input rejection is already fully specified by functional scenarios above.

See functional scenarios: [listPage rejects invalid pagination input](#scenario-listpage-rejects-invalid-pagination-input), [search rejects invalid query or limit input](#scenario-search-rejects-invalid-query-or-limit-input).
...
See functional scenario: [Switching between browsing and searching does not show stale results](#scenario-switching-between-browsing-and-searching-does-not-show-stale-results).
