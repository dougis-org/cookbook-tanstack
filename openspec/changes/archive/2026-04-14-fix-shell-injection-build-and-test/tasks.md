...
- [x] **Step 6 — Dry Run (Optional):** If a local runner like `act` is available, test the workflow locally. Otherwise, rely on CI validation after pushing.
...
- [x] **Unit tests** — `npm run test`; all tests must pass
- [x] **Integration tests** — `npm run test` (includes integration); all tests must pass
- [x] **Regression / E2E tests** — `npm run test:e2e`; all tests must pass
- [x] **Build** — `npm run build`; build must succeed with no errors
...
- [x] Archive the change: move `openspec/changes/fix-shell-injection-build-and-test/` to `openspec/changes/archive/2026-04-14-fix-shell-injection-build-and-test/` **and stage both the new location and the deletion of the old location in a single commit**
- [x] Confirm `openspec/changes/archive/2026-04-14-fix-shell-injection-build-and-test/` exists and `openspec/changes/fix-shell-injection-build-and-test/` is gone
- [x] Commit and push the archive to the default branch in one commit
- [x] Prune merged local feature branches: `git fetch --prune` and `git branch -d fix-shell-injection-build-and-test`
