...
- [x] 6.4 Verify CI uploads both `coverage/lcov.info` and `e2e-coverage/lcov.info` in the Codacy upload step logs
- [x] 6.5 Resolve any CI failures or review comments before merge

## 7. Post-Merge

- [ ] 7.1 Confirm Codacy dashboard reflects increased coverage after merge
- [x] 7.2 Run `/opsx:archive` to archive this change
- [x] 7.3 Sync approved spec deltas to `openspec/specs/` (update `github-actions-maintenance/spec.md`; create `e2e-coverage/spec.md`)
- [x] 7.4 Delete local feature branch: `git branch -d feat/e2e-coverage-reporting`
