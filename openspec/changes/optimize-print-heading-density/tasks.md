...
- [ ] Archive the change: move `openspec/changes/optimize-print-heading-density/` to `openspec/changes/archive/YYYY-MM-DD-optimize-print-heading-density/` **and stage both the new location and the deletion of the old location in a single commit** — do not commit the copy and delete separately
- [ ] Confirm `openspec/changes/archive/YYYY-MM-DD-optimize-print-heading-density/` exists and `openspec/changes/optimize-print-heading-density/` is gone
