...
Blocking dependency: this change cannot be applied until #632 (domain migration) is merged and #635 (`VITE_STRIPE_PUBLISHABLE_KEY` build-arg gap) is resolved, per the open question raised in `proposal.md`. Absent explicit override from the requester, `tasks.md` will include a gating task to verify both before the doc edit lands.
...
- [Risk] Applying before #632/#635 land bakes wrong values into the doc → [Mitigation] `tasks.md` includes an explicit verification/gating task before the doc-edit task; apply should not proceed past that gate without confirmation both are closed.
...
1. Confirm #632 merged (domain migration final state known).
2. Confirm #635 resolved (Stripe publishable-key storage location known).
3. Write the `## Production Secrets & Environment Variables` section into `docs/standards/ci-cd.md`.
4. Correct `.env.example`'s stale `APP_PRIMARY_URL` comment.
5. PR through normal CI/CD gates (this repo's standard flow — build, test, lint, security scans, Codacy — even though the diff is docs-only, per explicit user instruction to run this through the same gates as any other code change).

## Open Questions

- Carried from `proposal.md`: does apply wait on a hard confirmation that #632/#635 have both closed, or proceed with TBD placeholders and a final pass? `tasks.md` will assume "hard wait" unless told otherwise before apply begins.
