...
### Requirement: Security — production secrets documentation must not leak values

The system SHALL NOT include actual secret values (tokens, keys, URIs with credentials) anywhere in the production secrets documentation — only variable names, storage locations, and behavioral descriptions.

#### Scenario: No real secret values appear in the doc

- **Given** the "Production Secrets & Environment Variables" section
