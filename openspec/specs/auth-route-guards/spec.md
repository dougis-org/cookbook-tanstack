...
- Proposal "Redirects" scope item (`/auth/profile` and `/account/settings`
  ...
  links) → Requirements: Legacy `/auth/profile` route redirects authenticated
  visitors to `/account`, Legacy `/account/settings` route redirects
  authenticated visitors to `/account`
- Proposal acceptance criterion "Given an existing bookmark or link ... they
  ...
  legacy-redirect requirements above
