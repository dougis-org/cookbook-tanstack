# Spec: Recipe Soft Delete

## Capability
When a recipe is deleted, its document is retained in the database with `deleted: true` rather than being permanently removed. All application queries automatically exclude soft-deleted recipes via Mongoose pre-find middleware.

## Requirements

### R1 — Recipe delete sets deleted flag
...
### R2 — Soft-delete filter is automatic and schema-level
The system SHALL inject `{ deleted: { $ne: true } }` into all `find`, `findOne`, `findOneAndUpdate`, `countDocuments`, and `aggregate` queries on the Recipe model via Mongoose pre-find/aggregate middleware. Application code MUST NOT need to add this filter manually.

#### Scenario: Middleware filters without application changes
- **WHEN** any Recipe query runs (list, getById, ownership check, edit, aggregation)
- **THEN** soft-deleted recipes are excluded without any explicit filter in the calling code

### R3 — Backward compatibility with existing documents
...
### R4 — Soft-deleted recipe cannot be edited
