...
This document details *changes* to requirements and is additive to the [`design.md`](../../changes/archive/2026-06-29-notes-tier-change-integration-tests/design.md) document, not a replacement.
...
- **When** an admin calls `admin.users.setTier` to downgrade that user to `home-cook`, and the user's next `privateRecipeNotes.get` request is made with a session reflecting the persisted tier
...
- **When** an admin calls `admin.users.setTier` to upgrade the user back to `sous-chef`, and the user's next `privateRecipeNotes.get` request is made with a session reflecting the persisted tier
...
- **When** an admin calls `admin.users.setTier` to upgrade the user to `executive-chef`, and the user's next `privateRecipeNotes.get` request is made with a session reflecting the persisted tier
...
- **When** an admin calls `admin.users.setTier` to downgrade to `home-cook`, then calls `admin.users.setTier` to `home-cook` again
- **Then** the RecipeNote document remains identical to the pre-change snapshot — no fields were added, removed, or mutated
