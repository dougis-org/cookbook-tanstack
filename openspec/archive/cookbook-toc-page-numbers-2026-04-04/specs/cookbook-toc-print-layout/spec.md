...
- **THEN** the TOC sections are visually identical: same column layout, same chapter grouping, same numbering, same entry format, same dotted leaders, same page numbers
...
### Requirement: TOC entry displays estimated print page number with dotted leader

The system SHALL render each TOC entry as: index number, recipe name, a dotted leader line, and the estimated print page number — in that order. The dotted leader SHALL be a flex-1 span with a dotted bottom border that fills the space between the recipe name and the page number.
...
#### Scenario: TOC entry shows dotted leader and page number in print

- **WHEN** the TOC page is printed or shown in print preview
- **THEN** each recipe entry displays: index number, recipe name, a dotted leader line extending to the right margin, and the page number in full-contrast black
